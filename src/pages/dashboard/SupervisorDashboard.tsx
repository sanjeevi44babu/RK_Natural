  ...
  return (
    <DashboardLayout>
      <div className="p-4 lg:p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold">Admitted Patients ({admittedPatients.length})</h2>
          ...
          <div className="space-y-3">
            {admittedPatients.slice(0, 5).map((patient) => (
              <UserCard key={patient.id} id={patient.id} name={patient.fullName} subtitle={`${patient.age} Age, ${patient.gender} • Room ${patient.roomNumber || 'N/A'}`} phone={patient.phone} showActions onClick={() => navigate(`/patients/${patient.id}`)} />
            ))}
          </div>
        </div>
      </div>
