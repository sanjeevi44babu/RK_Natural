    ...
    <DashboardLayout>
      <div className="p-4 lg:p-6 space-y-6">
        ...
        <div className="space-y-3">
          <button onClick={() => navigate('/patients/new')} className="role-card w-full animate-fade-in hover:scale-[1.02] transition-transform duration-200" style={{ animationDelay: '0.3s' }}>
          ...
          {todayAppointments.length === 0 ? (
            ...
            <div className="space-y-3">
              {todayAppointments.slice(0, 5).map((appointment) => (
                <div key={appointment.id} className="card-medical">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-secondary/10 flex items-center justify-center">
                      <Stethoscope size={20} className="text-secondary" />
                    </div>
                    <div className="flex-1 cursor-pointer" onClick={() => navigate(`/appointments/${appointment.id}`)}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {appointment.status === 'upcoming' && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleCancelAppointment(appointment.id, appointment.patientName);
                          }}
                        >
                          <XCircle size={16} />
                        </Button>
        ...
        <div>
          ...
          {myPatients.length === 0 ? (
            <div className="card-medical text-center py-8">
              ...
              <button onClick={() => navigate('/patients/new')} className="mt-3 text-primary font-medium hover:underline">Add a new patient</button>
            </div>
          ) : (
            <div className="space-y-3">
      ...
      <button className="fab hover:scale-110 transition-transform duration-200 animate-scale-in" onClick={() => navigate('/patients/new')}>
        <Plus size={24} />
      </button>
    </DashboardLayout>
  );
