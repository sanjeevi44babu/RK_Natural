    ...
    <DashboardLayout>
      <div className="p-4 lg:p-6 space-y-6">
        ...
        <div className="space-y-3">
          {filtered.map((doc) => (
            <div key={doc.id} className="card-medical cursor-pointer" onClick={() => navigate(`/doctors/${doc.id}`)}>
              <div className="flex items-center gap-4">
                ...
                <p className="text-lg font-bold text-primary">{patients.filter(p => p.assignedDoctorId === doc.id).length}</p>
              </div>
            </div>
          ))}
        </div>
