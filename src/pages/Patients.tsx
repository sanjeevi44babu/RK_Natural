...
export default function Patients() {
  ...
  const filteredPatients = patients.filter(patient => {
    ...
    if (user?.role === 'doctor') {
      return matchesSearch && matchesStatus && patient.assignedDoctorId === user.id;
    }
    if (user?.role === 'physiotherapist') {
      return matchesSearch && matchesStatus && patient.assignedPhysiotherapistId === user.id;
    }
        ...
        <div className="space-y-3">
          {filteredPatients.length === 0 ? (
            ...
            filteredPatients.map((patient) => (
              <div key={patient.id} className="user-card" onClick={() => navigate(`/patients/${patient.id}`)}>
